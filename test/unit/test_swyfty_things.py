...
def test_valid_swyft_address():
    from swyftlib import is_valid_swyft_address

    main = valid_swyft_address()
    test = valid_swyft_address('testnet')

    assert is_valid_swyft_address(main) is True
    assert is_valid_swyft_address(main, 'mainnet') is True
    assert is_valid_swyft_address(main, 'testnet') is False

    assert is_valid_swyft_address(test) is False
    assert is_valid_swyft_address(test, 'mainnet') is False
    assert is_valid_swyft_address(test, 'testnet') is True


def test_invalid_swyft_address():
    from swyftlib import is_valid_swyft_address

    main = invalid_swyft_address()
    test = invalid_swyft_address('testnet')

    assert is_valid_swyft_address(main) is False
    assert is_valid_swyft_address(main, 'mainnet') is False
    assert is_valid_swyft_address(main, 'testnet') is False

    assert is_valid_swyft_address(test) is False
    assert is_valid_swyft_address(test, 'mainnet') is False
    assert is_valid_swyft_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from swyftlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % swyftlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(swyftlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(swyftlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(swyftlib.blocks_to_seconds(16616)) == 2612035
