...
class SwyftDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some swyftd methods
    ...
    @classmethod
    def from_swyft_conf(self, swyft_dot_conf):
        from swyft_config import SwyftConfig
        config_text = SwyftConfig.slurp_config_file(swyft_dot_conf)
        creds = SwyftConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = swyftlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = swyftlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = swyftlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_swyftd_proto_version_with_sentinel_ping)
