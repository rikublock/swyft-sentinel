...
def has_swyft_conf():
    ...
    valid_swyft_conf = False

    # ensure swyft_conf exists & readable
    #
    # if not, print a message stating that Swyft Core must be installed and
    # configured, including JSONRPC access in swyft.conf
    try:
        f = io.open(config.swyft_conf)
        valid_swyft_conf = True
    except IOError as e:
        print(e)

    return valid_swyft_conf
...
def main():
    ...
    if not has_swyft_conf():
        print("SwyftCore must be installed and configured, including JSONRPC access in swyft.conf")
        sys.exit(1)
