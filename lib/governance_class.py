...
class GovernanceClass(object):
    ...
    def vote(self, swyftd, signal, outcome):
        return self.go.vote(swyftd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, swyftd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(swyftd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(swyftd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import swyftlib
        obj_data = swyftlib.SHIM_serialise_for_swyftd(self.serialise())
    ...
    def swyftd_serialise(self):
        import swyftlib
        return swyftlib.SHIM_serialise_for_swyftd(self.serialise())
