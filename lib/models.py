...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, swyftd):
        golist = swyftd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_swyftd(swyftd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_swyftd(self, swyftd, rec):
        import decimal
        import swyftlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from swyftd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from swyftd, with every run
        ...
        # get/create, then sync payment amounts, etc. from swyftd - Swyftd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(swyftd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from swyftd! %s" % e)
            govobj.vote_delete(swyftd)
            return (govobj, None)
    ...
    def vote_delete(self, swyftd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(swyftd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, swyftd, signal, outcome):
        import swyftlib
        ...
        output = swyftd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = swyftlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(swyftd, signal)

    def sync_network_vote(self, swyftd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = swyftd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = SWYFTD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 swyft addr, non-multisig
            if not swyftlib.is_valid_swyft_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Swyft address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Swyft Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (swyftlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = SWYFTD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not swyftlib.is_valid_swyft_address(addr, config.network):
    ...
    def hash(self):
        import swyftlib
        return swyftlib.hashit(self.serialise())
